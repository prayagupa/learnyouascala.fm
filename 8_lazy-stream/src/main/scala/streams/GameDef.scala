...
trait GameDef {
  ...
  /**
    ...
    * smaller than `b2`.
    */
  case class Block(block1: Pos, block2: Pos) {

    // checks the requirement mentioned above
    require(block1.row <= block2.row && block1.col <= block2.col, "Invalid block position: b1=" + block1 + ", b2=" + block2)

    /**
      * Returns a block where the `row` coordinates of `b1` and `b2` are
      * changed by `d1` and `d2`, respectively.
      */
    def deltaRow(d1: Int, d2: Int) = Block(block1.deltaRow(d1), block2.deltaRow(d2))

    /**
      * Returns a block where the `col` coordinates of `b1` and `b2` are
      * changed by `d1` and `d2`, respectively.
      */
    def deltaCol(d1: Int, d2: Int) = Block(block1.deltaCol(d1), block2.deltaCol(d2))


    /** The block obtained by moving left */
    def left: Block = {
      if (isStanding) deltaCol(-2, -1)
      else if (block1.row == block2.row) deltaCol(-1, -2)
      else deltaCol(-1, -1)
    }

    /** The block obtained by moving right */
    def right: Block = {if (isStanding) deltaCol(1, 2)
    else if (block1.row == block2.row) deltaCol(2, 1)
    else deltaCol(1, 1)}

    /** The block obtained by moving up */
    def up: Block = {
      if (isStanding) deltaRow(-2, -1)
      else if (block1.row == block2.row) deltaRow(-1, -1)
      else deltaRow(-1, -2)
    }
    ...
    /**
      ...
      */
    def isStanding: Boolean = block1 == block2

    /**
      * Returns `true` if the block is entirely inside the terrain.
      */
    def isLegal: Boolean = terrain(block1) && terrain(block2)
  }
