...
object Anagrams {
   ...
   */
  def wordOccurrences(inputWord: Word): Occurrences = {
    val occurance = inputWord.toLowerCase.groupBy((character : Char) => character)
      .map(charToCharMap => (charToCharMap._1, charToCharMap._2.length))
      .toList
  ...
  def sentenceOccurrences(inputSentence: Sentence): Occurrences = {
    //inputSentence.map(word => wordOccurrences(word)).flatten
    wordOccurrences(inputSentence.foldLeft("")(_ + _))
  }
   ...
   */
  def sentenceAnagrams(inputSentence: Sentence): List[Sentence] = {
    ...
    val ana = sentenceAnagrams(sentenceOccurrences(inputSentence))
    println(ana)
    ana
  }
}
